...
def route_all(input_data):
    ...
    def _pad_density_at(x_mm, y_mm, win=3):
        """
        Count how many pad_mask cells exist in a small grid window
        around (x_mm, y_mm). Higher count = more crowded region.
        """
        ix, iy = grid.world_to_grid(x_mm, y_mm)
        total = 0
        nx = grid.nx

        for L in range(len(grid.layers)):   # check all layers (pads block on each)
            pm = grid.pad_mask[L]
    ...
    for t in tasks:
        ...
        d_start = _pad_density_at(s["x"], s["y"])
        d_goal  = _pad_density_at(g["x"], g["y"])
                ...
                try:
                    ...
                    for _, (ex_ix, ex_iy), (en_ix, en_iy) in pairs:
                        ...
                        try:
                            ...
                            # Convert path (ix,iy,layer) to world coords
                            world_path = []
                            for ix, iy, il in path_cells:
                                x, y = grid.grid_to_world(ix, iy)
                                world_path.append((round(x, 6), round(y, 6), grid.layers[il]))

                            # If we had an entry offset, keep your tiny tail into the true goal
                            if snap_tail_to_goal:
                                gx_true_tail, gy_true_tail = goal["x"], goal["y"]
                                last_layer_name = world_path[-1][2]
                                world_path.append((round(gx_true_tail, 6), round(gy_true_tail, 6), last_layer_name))

                            # --- NEW: robust snap + verify for pad contact ---

                            # True pad positions from the original task
                            sx_true, sy_true = start["x"], start["y"]
                            ...
                            # How close we require the path to get before we allow snapping (mm)
                            MAX_PAD_ATTACH_DIST = float(rules_net.get("max_pad_attach_dist", 0.5))

                            # Current start & end (after tail, if any)
                            x0, y0, l0 = world_path[0]
                            xN, yN, lN = world_path[-1]

                            # If the path actually reached near the pad, snap the endpoints into pad center
                            if _dist(x0, y0, sx_true, sy_true) <= MAX_PAD_ATTACH_DIST:
                                world_path[0] = (round(sx_true, 6), round(sy_true, 6), l0)

                            if _dist(xN, yN, gx_true, gy_true) <= MAX_PAD_ATTACH_DIST:
                                world_path[-1] = (round(gx_true, 6), round(gy_true, 6), lN)

                            # Re-check distances after snapping
                            x0, y0, l0 = world_path[0]
                            xN, yN, lN = world_path[-1]
                            dist_start = _dist(x0, y0, sx_true, sy_true)
                            dist_goal  = _dist(xN, yN, gx_true, gy_true)

                            # If we never got close enough to one of the pads, treat this as a failed route
                            if dist_start > MAX_PAD_ATTACH_DIST or dist_goal > MAX_PAD_ATTACH_DIST:
                                raise RuntimeError(
                                    f"Path did not reach pad (start_dist={dist_start:.3f}, "
                                    f"goal_dist={dist_goal:.3f})"
                                )

                            # Now we know both ends truly attach to their pads -> safe to compress
